...
{
	printf("\n\tUsage: %s <infile1> [infile2] .. [-o/--output outfile]\n\n",
			progname);
	printf("%s takes one or many input files and an optional\n", progname);
	printf("output file specified by -o or --ouput option. The input files\n"
		"should be transliterated text files generated by pallavi_ml2p\n"
		"It is a good idea to use all the input trans files here before\n"
		"dictionary generation. This will prevent duplicate words in the\n"
		"dictionary\n\n");
}
...
int main(int argc, char *argv[])
{
	...
	while (optind < argc) {
		if (argv[optind][0] == '-') {
			if (!strcmp(argv[optind], "-o")) {
				outfile = optind+1;
				break;
			} else if (!strcmp(argv[optind], "--output")) {
				outfile = optind+1;
				break;
			} else {
	...
	/* Process all the input files in the arguments. We have the option and
	 * output file name in optind and outfile. Skip them if any */
	while (arg < argc) {
		if ((arg == outfile) || (arg == (outfile-1))) {
			arg++;
			continue;
		}
	...
	fflush(ofp);
	printf("Output word list generated in %s\n", ofile);
